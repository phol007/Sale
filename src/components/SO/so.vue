					...
					<tbody>
	                    <tr v-for="(item_list, index) in detail_itemlists" @dblclick="selectTR(item_list)" class="item_list_label" @click="show_stock(index, item_list)">
                        	...
                        	<td style="width: 100px; padding:0 0.5%; text-align:center;">
                    			<select v-model="stocks[index] = item_list.stock_select" @change="calcItemAmount(index, unit_list, item_list.qty, '', item_list.discount, item_list.amount, stocks)">
                        	...
                        	<td style="width: 100px; padding:0 0.5%;">
                        		<select v-model="unit_list[index] = item_list.unit_select"  style="cursor: pointer;" @change="calcItemAmount(index, unit_list, item_list.qty, '', item_list.discount, item_list.amount, stocks)">
                        			<option v-for="unit in item_list.units" :value="unit">{{ unit.unit_name }}</option>
                        		</select>
                        	</td>                       	
                        	<td style="padding:0;">
                        		<input type="text" v-model="item_list.qty" placeholder="0.00" @change="calcItemAmount(index, unit_list, item_list.qty, item_list.price, item_list.discount, item_list.amount, stocks)" @click="return_Int_Item(index, item_list.qty, '', '')" @focus="return_Int_Item(index, item_list.qty, '', '')" @blur="return_FM_Item(index, item_list.qty, item_list.price, item_list.discount)" @keydown="keyNumber">
                        	</td>
                        	<td style="padding:0;">
                        		<input type="text" placeholder="0.00" v-model="item_list.price" @change="calcItemAmount(index, unit_list, item_list.qty, item_list.price, item_list.discount, item_list.amount, stocks)" @click="return_Int_Item(index, '', item_list.price, '')" @focus="return_Int_Item(index, '', item_list.price, '')" @blur="return_FM_Item(index, item_list.qty, item_list.price, item_list.discount)" @keydown="keyNumber">
                        	</td>
                        	<td style="padding:0;">
                        		<input type="text" placeholder="0%, 0.00" v-model="item_list.discount" @change="calcItemAmount(index, unit_list, item_list.qty, item_list.price, item_list.discount, item_list.amount, stocks)" @click="return_Int_Item(index, '', '', item_list.discount)" @focus="return_Int_Item(index, '', '', item_list.discount)" @blur="return_FM_Item(index, item_list.qty, item_list.price, item_list.discount)" @keydown="keyNumber">
                        	</td>
                        	<td style="padding:0;">
                        		<input type="text" placeholder="0.00" :value="formatMoney(item_list.amount)" readonly>
                        	</td>
	                    </tr>
					</tbody>
				</table>
